...
| 项目 | 实测值 |
| --- | --- |
| HTTP 状态 | 正常时 200 稳定；源站限流按 **IP** 计算，见 15.2 |
...
feed 新鲜度有一次测得准的观测：在轮询间隔约 0.25 秒的高频探测下，新帖**首次**出现在 feed 里时，
它的 `pubDate` 已经过去 **11.9 秒**（同一行 `new=1`，说明正是首次见到该帖）。
0.25 秒相对这个数字可以忽略，所以可以认为 **feed 自身的新鲜度约 12 秒**。

据此，端到端最好情况约 12 + 15 + 1~2 ≈ **25~30 秒**，与「30 秒」的目标同量级。
注意这是**观察到的下限，不是承诺**：限流退避、源站抖动、TG 投递都会让它更长。
...
上线前的一次压力实测跑了约 18,000 次请求（脚本无 sleep，单次响应约 230 ms，
即持续约 4 次/秒）。**当时抽样看到的全是 200**，据此曾判断源站容忍度有很大余量。
**这个结论是错的。** 回看那次实测留下的逐请求 CSV（列依次为
`时间戳, status, 字节数, 耗时ms, item数, 新增数, 最新帖id, 最新帖年龄s, 最新帖标题, 错误`），
同一次运行的尾部已经混进了两类失败：

    2026-09-21T19:21:48+00:00,,,54,,,,,,HTTP Error 429: Too Many Requests      <- CF 边缘直接拒，约 55 ms
    2026-09-21T19:21:50+00:00,,,227,,,,,,HTTP Error 503: Service Unavailable  <- 源站过载，约 230 ms

- `429` 由 Cloudflare 边缘给出，回得很快（约 55 ms）；`503` 是源站被打到过载才给的。
- 两类错误交替出现，说明 18,000 次不是「安全余量」，而是把整个 IP 送进了限流状态。
- 上线后正常的 30 秒轮询随即收到 `429`，根因是同一台机器上**还有另一个进程在抓同一个 feed**：
...
1. **feed 新鲜度下限只有单次实测** — 唯一测得准的一次是 11.9 秒（见 3.4），样本只有一条。
   别把「最新帖年龄」当新鲜度指标：它实测在 40 秒到 7 分钟之间波动，衡量的是**距上一篇帖子的间隔**，
   凌晨没人发帖时会自然涨上去，所以只能拿它做溢出/活性判断，不能做延迟告警（见 11.3）。
